...
<?php
    ...
    $sql = "SELECT email FROM jsh_member WHERE email = '{$email}'";
    ...
    $sql = "SELECT nickName FROM jsh_member WHERE nickname = '{$nickName}'";
    // $result = $dbConnect->query($sql);
    $result = sql_query($sql);
    ...
    if( $result ) {
        ...
        echo "에러발생2 : 관리자 문의 요망";
        exit;
    }

    if ($isEmailCheck == true && $isNickNameCheck == true) {
        $regTime = time();
        $sql = "INSERT INTO jsh_member(email, nickname, pw, birthday, regTime)";
        ...
        if ($result) {
            $sql2 = "SELECT myMemberID FROM jsh_member WHERE email = '{$email}'";   //아래의 insert_id를 지금은 사용하지 못해서 SELECT문으로 대체해서 기능 되도록 함.(230706)
            ...
            //$_SESSION['memberID'] = $dbConnect->insert_id;  //$dbConnect->insert_id 여기서 insert_id를 쓰면 $result가 true일 때니까 위에 insert문이 true라는 의미.
                                                            //회원가입 창이고, 테이블 구조를 보면 memberId가 있는데 위에 insert문에서는 안 썼다. 왜냐면 자동으로 생성되니까(auto_increment)
                                                            ...
                                                            //이 줄의 우변에 넣어서 값을 대입해주면 원래 하려고 하는 기능이 작동 될 것. (230706)
